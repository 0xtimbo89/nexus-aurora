...
contract NexusProtocol_v1 is Ownable {
    uint256 private _listingCount;
    uint256 private _listingCompleted;

    mapping(bytes32 => Listing) private listingCatalog;
    ...
    function createListing(
        ...
        require(
            ITRC721(_collectionAddress).ownerOf(_tokenId) == msg.sender,
            "Seller must be owner of token"
        );

        require(
            ITRC721(_collectionAddress).isApprovedForAll(
                msg.sender,
                address(this)
            ),
        ...
        payable
    {
        ...
        ITRC721(_collectionAddress).transferFrom(
            listingToUpdate.seller,
            msg.sender,
            _tokenId
        );
